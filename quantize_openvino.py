...
def _preprocess_bgr(frame_bgr: np.ndarray, img_size: int) -> np.ndarray:
...
def _get_input_name(ov_model) -> str:
    try:
    ...
    names = list(input_port.get_names())
    if names:
        return names[0]
    return "images"
...
def main() -> None:
    ...
    for path in image_paths:
        ...
        if image is None:
            continue
        calibration_inputs.append(_preprocess_bgr(image, img_size))
    ...
    if not args.no_verify_runtime:
        ...
        verify_input_name = _get_input_name(compiled_model)
