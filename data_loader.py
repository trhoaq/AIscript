...
def _bbox_params():
    # Albumentations 2.x: aggressively drop invalid/degenerate boxes to keep
    # geometric transforms and dropout ops numerically stable.
    return A.BboxParams(
        format="pascal_voc",
...
def get_base_transforms(img_size=256):
    ...
    return A.Compose([
        A.PadIfNeeded(min_height=1000, min_width=1000, border_mode=cv2.BORDER_CONSTANT, fill=0, p=1.0),
...
def get_eval_transforms(img_size=256):
    return A.Compose([
        ...
        A.PadIfNeeded(min_height=img_size, min_width=img_size, border_mode=cv2.BORDER_CONSTANT, fill=0, p=1.0),
        A.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        ToTensorV2(),
    ], bbox_params=_bbox_params())
