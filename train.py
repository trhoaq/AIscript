...
from core.trainer import DetectorTrainer
from core.wandb_utils import finish_wandb, init_wandb, log_wandb
...
def load_teacher_weights(teacher: SSDMobile, checkpoint_path: str, device: torch.device) -> None:
    ...
    checkpoint: Dict[str, Any] = torch_load_checkpoint(checkpoint_path, map_location=device)
...
def main():
    ...
    task = str(config.get("task", "detect")).strip().lower()
    if task not in {"detect", "detection"}:
        finish_wandb()
        raise ValueError("task must be one of: detect, detection")
